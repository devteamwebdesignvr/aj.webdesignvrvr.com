...
<div class="banner">
    ...
    <div class="guides d-none">
        <h1 class="c-hero__title">Get a glimpse of Lowcountry living.</h1>
        <p>Come on down, y’all. We can’t wait to host you.</p>
    </div>
</div>
        ...
        <form method="get" action="{{ url('properties')}}">
            <div class="row">
                <div class="col-3 md-12 sm-12 select">
                    {!! Form::select("location_id",ModelHelper::getLocationTrueSelectList(),Request::get("location_id"),["class"=>"","placeholder"=>"Select Location","title"=>"Select Location","id"=>"loc"]) !!}
                    <i class="fa-solid fa-location-dot"></i>
                </div>
                <div class="col-6 md-12 sm-12 main-check">
                    <div class="row">
        ...
        <div class="row">
            @foreach($list as $c)
                @php
                    ...
                    $address = json_decode($data->address_object);
                @endphp
                <div class="col-4 col-md-4 col-sm-12">
                    <div class="pro-img">
                        @if($c->feature_image)
                        <a href="{{ url($c->seo_url).'?'.http_build_query(request()->all()) }}"><img src="{{asset($c->feature_image)}}" class="img-fluid" alt=""></a>
                        @else
                        @isset($rightImage)
                         @foreach($rightImage as $limg)
                         <a href="{{ url($c->seo_url).'?'.http_build_query(request()->all()) }}"><img src="{{asset($limg->url)}}" class="img-fluid" alt="{{$limg->caption ?? ''}}"></a>
                         @endforeach
                        @endisset
                        @endif
                    </div>
                    <div class="pro-cont">
                        <h3 class="title"><a href="{{ url($c->seo_url).'?'.http_build_query(request()->all()) }}">{{$c->title}}</a></h3>
                         @if ($address)
                        <p class="adr">
                             {{ $address->city ?? '' }}@if($address->city), @endif {{ $address->state ?? '' }}
                        </p>
                        @endif

                      
                        <p>{!! Str::limit($c->summary,400) !!}</p>
                        <div class="amn">
                            <ul class="first">
                                @if($c->capacity_bedrooms)<li><i class="fa-solid fa-bed"></i> {{ $c->capacity_bedrooms }} Bedrooms</li>@endif
                               @if($c->capacity_bathrooms) <li><i class="fa-solid fa-bath"></i> {{ $c->capacity_bathrooms }} Bathrooms</li> @endif
                               @if($c->capacity_max) <li><i class="fa-solid fa-users"></i> {{ $c->capacity_max }} Guests</li> @endif
                            </ul>
                        </div>
                        <div class="view">
                            <!-- @isset($prices['basePrice'])
                            <h5 style="visibility: hidden;"><span>$ {{ $prices['basePrice']  }}199</span> / Night</h5>
                            @endisset -->
...
@endif
<div class="map-sec d-none">
    <h3>Find the perfect spot for your Charleston getaway</h3>
    <iframe src="{!! $setting_data['property-list-map'] ??'' !!}" width="100%" height="88%" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
</div>
@stop
