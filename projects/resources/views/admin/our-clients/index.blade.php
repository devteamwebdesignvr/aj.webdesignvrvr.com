...
@section('title', 'Admin')
@php 
    $name="Amenities Includes";$route="our-clients";
@endphp            
                ...
                <tbody>
                    ...
                    @foreach($data as $client)
                        <tr>
                          ...
                          <td>
                                    @if($client->icon)
                                          <img src="{{ asset($client->icon) }}" width="100" > 
                                    @endif
                            </td>
...
<script>
  $("#data-table-gaurav").DataTable();
   $(document).on("click",".delete-checkbox",function(){
      ajaxCallCheckBoxDelete("{{ route('multipleDelete','our-clients') }}","{{ route('our-clients.index') }}","our-clients");
  });
</script>
@stop