...
@section('title', 'Admin')
@php 
    $name="Home Gallery";$route="home_galleries";
@endphp            
                ...
                <tbody>
                    ...
                    @foreach($data as $client)
                        <tr>
                            ...
                            <td>{{ App\Models\Cms::find($client->type)->name ?? $client->type }}</td>                               
                            <td>
...
<script>
  ...
  $(document).on("click",".delete-checkbox",function(){
      ajaxCallCheckBoxDelete("{{ route('multipleDelete','galleries') }}","{{ route('galleries.index') }}","galleries");
  });
