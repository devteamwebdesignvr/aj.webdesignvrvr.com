...
class EmailTempleteController extends Controller{
    ...
    public function __construct(EmailTemplete $model){
        $this->model=$model;
        $this->admin_base_url="email-templetes.index";
        $this->admin_view="admin.email-templetes";
    }
    ...
    public function store(Request $request){
        $data=$request->all();
        if($request->hasFile("image")) {$data['image'] = Upload::fileUpload($request->file("image"),"email-templetes");}
        if($request->hasFile("bannerImage")) {$data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"email-templetes");}
