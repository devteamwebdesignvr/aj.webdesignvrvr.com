...
class SeoCmsController extends Controller{
    ...
    public function __construct(SeoCms $model){
        $this->model=$model;
        $this->admin_base_url="seo_pages.index";
        $this->admin_view="admin.seo_pages";
    }
    ...
    public function store(Request $request){
        $validator = Validator::make($request->all(), ['seo_url' => 'required|unique:seo_pages,seo_url','name' => 'required']);   
        if($validator->fails()){
    ...
    public function update(Request $request, $id){
        $validator = Validator::make($request->all(), ['seo_url' => 'required|unique:seo_pages,seo_url,'.$id,'name' => 'required']);   
        if($validator->fails()){
            return redirect()->back()->withInput()->with("danger",$validator->errors()->first())->withErrors($validator->errors());
        }
        $exist=$this->model::find($id);
        if($exist){
            $dta=$request->all();if($request->hasFile("image")) {$data['image'] = Upload::fileUpload($request->file("image"),"cms");}if($request->hasFile("vacation_four_image")) {$data['vaation_four_image'] = Upload::fileUpload($request->file("vacation_four_image"),"cms");}if($request->hasFile("vacation_three_image")) {$data['vacation_three_image'] = Upoad::fileUpload($request->file("vacation_three_image"),"cms");}if($request->hasFile("vacation_two_image")) {$data['vacation_two_image'] = Upload::fileUpload($request->fie("vacation_two_image"),"cms");}if($request->hasFile("vacation_one_image")) {$data['vacation_one_image'] = Upload::fileUpload($request->file("vacation_one_image"),"cms");if($request->hasFile("bannerImage")) {$data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"cms");}
            $attraction_secion=[];
            $attraction_secion=[];
            if($request->attraction_heading){
                foreach($request->attraction_heading as $key=>$v){
