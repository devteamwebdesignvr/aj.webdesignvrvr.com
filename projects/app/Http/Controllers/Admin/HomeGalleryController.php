...
class HomeGalleryController extends Controller{
    ...
    public function __construct(HomeGallery $model){
        $this->model=$model;
        $this->admin_base_url="home_galleries.index";
        $this->admin_view="admin.home_galleries";
    }
    ...
    public function store(Request $request){
        $data=$request->all();
        if($request->hasFile("image")) {$data['image'] = Upload::fileUpload($request->file("image"),"galleries");}
        if($request->hasFile("bannerImage")) {$data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"galleries");}
