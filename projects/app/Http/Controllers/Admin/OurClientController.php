...
class OurClientController extends Controller{
    ...
    public function store(Request $request){
        $data=$request->all();
        if($request->hasFile("image")) {$data['image'] = Upload::fileUpload($request->file("image"),"our-clients");}
        if($request->hasFile("icon")) {$data['icon'] = Upload::fileUpload($request->file("icon"),"our-clients");}
        if($request->hasFile("bannerImage")) {$data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"our-clients");}
