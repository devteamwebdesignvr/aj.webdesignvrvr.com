...
class PropertyEnquiryRequestController extends Controller{
    
    public function __construct(PropertyEnquiryRequest $model){
        $this->model=$model;
        $this->admin_base_url="property_enquiry_requests.index";
        $this->admin_view="admin.property_enquiry_requests";
    }
    ...
    public function store(Request $request){
        ...
        if ($request->hasFile("image")) {
            $data['image'] = Upload::fileUpload($request->file("image"),"property_enquiry_requests");
        }
        if ($request->hasFile("bannerImage")) {
            $data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"property_enquiry_requests");
        }
