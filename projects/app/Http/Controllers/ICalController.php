...
END:VEVENT\n";
   ...
   function sendReminderPackage(){
        ...
        $events = BookingRequest::where(["booking_status"=>"booking-confirmed","payment_status"=>"partially","total_payment"=>3,"how_many_payment_done"=>1,"third_reminder_email"=>"false","reminder_email"=>"false","booking_type_admin"=>"invoice"])->where("checkin",$date)->get();
        foreach($events as $data){
            $property=Property::find($data->property_id);
            if($property){
                $html= view("mail.reminder-admin-email",compact("property","data"))->render();
        ...
        $events = BookingRequest::where(["booking_status"=>"booking-confirmed","payment_status"=>"partially","total_payment"=>3,"how_many_payment_done"=>2,"third_reminder_email"=>"false","booking_type_admin"=>"invoice"])->where("checkin",$date)->get();
        foreach($events as $data){
            $property=Property::find($data->property_id);
            if($property){
                $html= view("mail.reminder-admin-email",compact("property","data"))->render();
