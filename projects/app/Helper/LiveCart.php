...
class LiveCart
{
    ...
    function iCalDataCheckInCheckOut($property_id)
    {
        ...
        // }
        $property = GuestyProperty::find($property_id);


        sort($checkin);
        sort($checkout);
        if ($property) {
            $unavailable_dates = GuestyAvailablityPrice::where(["listingId" => $property->_id])->whereIn("status", ["unavailable", "booked", "reserved"])->get();
            $un_dates = [];
            foreach ($unavailable_dates as $u) {
                //one day after
                $date = date('Y-m-d', strtotime("+ 1 days", strtotime($u->start_date)));
                $single_date_avilablity = GuestyAvailablityPrice::where(["listingId" => $property->_id])->where("start_date", $date)->whereIn("status", ["available"])->first();
                if ($single_date_avilablity) {
                    $checkout[] = $single_date_avilablity->start_date;
                } else {
                }
                ...
                if ($single_date_avilablity) {
                    ...
                    $checkout[] = $u->start_date;
                }


                $checkin[] = $u->start_date;
                //$checkout[] =$u->start_date;
            }
            sort($checkin);
            sort($checkout);
    ...
    function iCalDataCheckInCheckOutCheckinCheckout11($property_id)
    {
        $data = IcalEvent::where(['event_pid' => $property_id])->where("end_date", ">", date('Y-m-d'))->get();
        $checkin = [];
        $checkout = [];
        ...
        $property = GuestyProperty::find($property_id);
        if ($property) {
            $data = GuestyPropertyBooking::where(['listingId' => $property->_id])->where("end_date", ">", date('Y-m-d'))->get();
            $checkin = [];
            $checkout = [];
            foreach ($data as $row) {
                $checkin[] = $row->start_date;
        ...
        if ($property) {
            $unavailable_dates = GuestyAvailablityPrice::where(["listingId" => $property->_id])->whereIn("status", ["unavailable"])->get();
            $un_dates = [];
            foreach ($unavailable_dates as $u) {
                $blocked[] = $u->start_date;
            }
            sort($checkin);
            sort($checkout);
        }
    ...
    function refreshIcalData($property_id, $ical_link, $id)
    {
        ...
        foreach ($icals as $c) {
            $ar = ['ppp_id' => $property_id, 'ical_link' => $ical_link, 'start_date' => date('Y-m-d', strtotime($c['start_date'])), 'end_date' => date('Y-m-d', strtotime($c['end_date'])), 'text' => $c['text'], 'event_pid' => $property_id, 'cat_id' => $id, 'uid' => $c['event_id'], 'event_type' => "ical", 'booking_status' => 1];
            $ar1 = ['ppp_id' => $property_id, 'ical_link' => $ical_link, 'start_date' => date('Y-m-d', strtotime($c['start_date'])), 'end_date' => date('Y-m-d', strtotime($c['end_date'])), 'event_pid' => $property_id, 'cat_id' => $id,];
    ...
    function allIcalImportListRefresh()
    {
        ...
        foreach ($data as $ical_list) {
            $this->refreshIcalData($ical_list->property_id, $ical_list->ical_link, $ical_list->id);
        }
        foreach (Property::all() as $c) {
            $this->getFileIcalFileData($c->id);
        }
    }
    ...
    function simpleParseXML($string)
    {
        ...
        for ($i = 0; $i < sizeof($xmlobj->event); $i++) {
            $a[$i] = array('event_id' => (string)$xmlobj->event[$i]->attributes(), 'start_date' => (string)$xmlobj->event[$i]->start_date, 'end_date' => (string)$xmlobj->event[$i]->end_date, 'text' => (string)$xmlobj->event[$i]->text, 'rec_type' => (string)$xmlobj->event[$i]->rec_type, 'event_pid' => (string)$xmlobj->event[$i]->event_pid, 'event_length' => (string)$xmlobj->event[$i]->event_length);
        }
        return $a;
    }
    ...
    function toHash($str)
    {
        if (strpos($str, "BEGIN:VCALENDAR") === false) {
            $ctx = stream_context_create(array('http' => array('method' => 'GET', 'header' => 'User-Agent: Mozilla/5.0 (Windows NT 6.3; WOW64; Trident/7.0; Touch; rv:11.0) like Gecko')));
    ...
    function toHashnew($str)
    {
        if (strpos($str, "BEGIN:VCALENDAR") === false) {
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_HTTPHEADER => array("cache-control: no-cache", "postman-token: edf0746d-6412-8902-fb96-a034976710e8")
            ));
            $str = curl_exec($curl);
            $err = curl_error($curl);
            curl_close($curl);
